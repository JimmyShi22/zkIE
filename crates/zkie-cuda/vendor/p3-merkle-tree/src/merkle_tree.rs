    ...
    MerkleTree<F, W, M, N, DIGEST_ELEMS>
{
    ...
    /// Build a tree from externally computed digest layers.
    ///
    /// zkIE-only extension (see the vendored-crate note in the manifest):
    /// `zkie-cuda` computes `digest_layers` on the GPU with kernels that are
    /// bit-compatible with `first_digest_layer`/`compress`, then calls this to
    /// obtain a tree whose `open`/`verify` behavior is identical to the CPU
    /// build. The caller must supply layers whose geometry matches
    /// [`MerkleTree::new`] exactly: `digest_layers[0]` holds one digest per
    /// row of the tallest matrix (padded with default digests), each later
    /// layer is the compression of the previous one, and `arity_schedule[i]`
    /// is the step used between layers `i` and `i + 1`.
    #[doc(hidden)]
    pub fn from_parts(
        leaves: Vec<M>,
        ...
        assert!(!digest_layers.is_empty(), "no digest layers given");
        assert_eq!(
            digest_layers.last().unwrap().len(),
            1,
            "the final digest layer must contain exactly one root digest"
        );
        assert_eq!(
            arity_schedule.len() + 1,
            digest_layers.len(),
            "one arity per layer transition"
        );
        Self {
            leaves,
            digest_layers,
