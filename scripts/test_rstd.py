#!/usr/bin/env python3
"""Check whether the rsqrt lookup (fixed index scale 2^14) is the fidelity bottleneck."""
import re
...
def rms_norm(x, w, rsqrt_t, mode):
    if mode == "q":
        sq = (x**2).sum(axis=-1)
        s = dr(sq, H)
        rstd = rsqrt_t[dr(s, 1 << 18)][:, None]
        return rs(x * rstd * w, 32)
    xf = x.astype(np.float64) / SCALE
    rstd = 1 / np.sqrt((xf * xf).sum(-1, keepdims=True) / H + 1e-6)
    return q(xf * rstd * w)


def layer_norm(x, w, b, rsqrt_t, mode):
    if mode == "q":
        mean = dr(x.sum(axis=-1), H)
        var = dr(((x - mean[:, None]) ** 2).sum(axis=-1), H)
        rstd = rsqrt_t[dr(var, 1 << 18)][:, None]
        return rs((x - mean[:, None]) * rstd * w, 32) + b
    xf = x.astype(np.float64) / SCALE
    mean = xf.mean(-1, keepdims=True)
    var = ((xf - mean) ** 2).mean(-1, keepdims=True)
    return q((xf - mean) / np.sqrt(var + 1e-6) * w + b)
...
def main():
    ...
    rsqrt_t = np.fromfile(f"{base}/rsqrt_table_i32.bin", dtype=np.int32).astype(np.int64)
    ...
    onnx.save(m4, "/tmp/rstd_ref.onnx")
    s4 = ort.InferenceSession("/tmp/rstd_ref.onnx", providers=["CPUExecutionProvider"])
    ...
    refq = q(ts[0])
    mean = dr(refq.sum(), 32)
    rstd = int(rsqrt_t[dr(dr(((refq - mean) ** 2).sum(), 32), 1 << 18)])
    ...
    onnx.save(m5, "/tmp/rstd_freq.onnx")
    s5 = ort.InferenceSession("/tmp/rstd_freq.onnx", providers=["CPUExecutionProvider"])
    ...
    def run(mode):
        ...
        for L in range(20):
            ...
            lnw_f = inits[f"{p}.input_layernorm.weight"]
            lnw = q(lnw_f)
            residual = x
            x = rms_norm(x, lnw if mode == "q" else lnw_f, rsqrt_t, mode)
            ...
            mlp_w_f = inits[f"{p}.mlp.layer_norm.weight"]
            mlp_b_f = inits[f"{p}.mlp.layer_norm.bias"]
            mlp_w = q(mlp_w_f)
            mlp_b = q(mlp_b_f)
            ln = layer_norm(x, mlp_w if mode == "q" else mlp_w_f, mlp_b if mode == "q" else mlp_b_f, rsqrt_t, mode)
    ...
    print("rsqrt table (index 2^14) err =", run("q"))
    print("float rstd              err =", run("f"))
